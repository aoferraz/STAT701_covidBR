...
[plot Brazil map with states and number of beds]
...
[plot Brazil map with states and votes on JB]
...
[add figure of information flow]

The regression script formats the cleaned and joined data in order to perform the regression. Because of the huge number of independent variables, instead of running a full LASSO, we ran regression within each category to screen variables, eliminate potential colinearities, remove variables with excessive NA or NaN values, pre-selecting variables for the full LASSO + fine tuning.
