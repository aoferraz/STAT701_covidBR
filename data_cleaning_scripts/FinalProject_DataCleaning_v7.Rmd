...
# Save cleaned data
metro %>%
  write.csv("data_cleaned/db_metro.csv",row.names=FALSE)
```
...
## Beneficiaries of the 'Bolsa Família' program (governemnt familly allowance)

Source: Dados.gov (2019), https://dados.gov.br/dataset/bolsa-familia-misocial

```{r}
# Beneficiaries of Bolsa Familia in 2019 (federal financial aid for poor families)
bolsa.familia <- read_xlsx("data/Dados.gov/BolsaFamilia-2019.xlsx") #local path of database
#str(bolsa.familia)
a <- dim(bolsa.familia)
...
bolsa.familia <- bolsa.familia[,c(5,3,4)]
#str(bolsa.familia)

# Match with population data set
bolsa.familia <- bolsa.familia %>%
  left_join(pop, by = "city_code") %>%
    mutate(bolsa.familia_familiesPer1Khab = bolsa.familia_families / pop.2019 * 1000) %>%
      mutate(bolsa.familia_valuePerHab = bolsa.familia_value / pop.2019)
#str(bolsa.familia)

bolsa.familia <- bolsa.familia[,c(1,15,16)]
str(bolsa.familia)

# Save cleaned data
bolsa.familia %>%
  write.csv("data_cleaned/db_bolsa_familia.csv",row.names=FALSE)
```
...
spec_beds <- spec_beds[,c(1,2:10,19:27,36:44,53:61)] # WRONG!!!!!!!
